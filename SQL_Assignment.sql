-- =============================================================================
-- SQL ASSESSMENT SOLUTIONS: Q1 - Q20
-- =============================================================================
...
CustomerBrackets AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN total_spend >= 5000 AND total_spend < 20000 THEN 'Mid Spender'
            ELSE 'Low Spender'
...
WITH RECURSIVE DateSeries AS (
    ...
    UNION ALL
    SELECT CAST(calendar_date + INTERVAL '1 DAY' AS DATE)
    FROM DateSeries
...
FROM DateSeries ds
LEFT JOIN orders o 
    ON ds.calendar_date = CAST(o.order_date AS DATE)
...
SELECT 
    order_month,
    total_revenue,
    LAG(total_revenue, 1) OVER (ORDER BY order_month ASC) AS previous_month_revenue,
    total_revenue - LAG(total_revenue, 1) OVER (ORDER BY order_month ASC) AS mom_dollar_growth
FROM MonthlyRevenue
...
SELECT 
    ...
    CAST(order_date AS DATE) - CAST(LAG(order_date) OVER (
        PARTITION BY customer_id 
        ORDER BY order_date ASC
    ) AS DATE) AS days_since_previous_order
FROM orders
...
SELECT 
    ...
    ) AS running_cumulative_net_revenue
FROM orders
WHERE order_status = 'Completed';
...
SELECT 
    ...
    product_revenue,
    ROUND(
        100.0 * product_revenue / SUM(product_revenue) OVER (PARTITION BY category_name),
        2
...
SELECT 
    ...
    department_id,
    salary,
    MAX(salary) OVER (PARTITION BY department_id) AS highest_dept_salary,
    MAX(salary) OVER (PARTITION BY department_id) - salary AS salary_difference
FROM employees;
